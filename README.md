...
*   **通信の安定性向上と誤切断バグの修正**
    ...
    * ESP32のマルチコア（非同期処理）によるレースコンディションが原因で、パケット受信時刻と判定時刻が競合して「時間のアンダーフロー（巻き戻り）」が発生し、超巨大な経過時間として誤検知されて強制切断されるバグを特定・修正しました。
...
*   **WebUIからSSIDとパスワードを書き換える処理の追加**
    *   ユーザー側でWi-FiのSSIDとパスワードを変更できるようにWebUI(192.168.4.1)へアクセスできるよう追加。
    *   入力のミスや、パスワード忘れへの対応のためBootボタン7回連続入力で初期値へリセットする処理を追加。
    
*   **トラッカーペアリング情報のリセット調整**
    *   Bootボタン5回でトラッカーペアリング情報のリセットを行う処理に追加した記憶機能も連動するように調整。
